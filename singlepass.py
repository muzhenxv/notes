...
spark = SparkSession.builder. \
    ...
    appName("singlepass_cluster_user_registration_fraud_identification"). \
...
def getGeohashCode(row, LatColName, LonColName):
    lat, lng = row[LatColName], row[LonColName]
    if len({str(lat).lower(), str(lng).lower()} & {'unknown', 'null', 'nan', 'none', '(null)', '(nan)', '(none)', '未知', '', ' ', '0'}) == 0:
...
def getSubString(row, colName, start, lenght):
    content = str(row[colName])

    if content.lower() in {'unknown', 'null', 'nan', 'none', '(null)', '(nan)', '(none)', '未知', '', ' ', '0'}:
        subContent = 'nan'
    else:
...
def getConcatFeature(row, ColName_1, ColName_2):
    content_1 = str(row[ColName_1])
    content_2 = str(row[ColName_2])

    if content_1.lower() in {'unknown', 'null', 'nan', 'none', '(null)', '(nan)', '(none)', '未知', '', ' '}:
        content_1 = ''
    if content_2.lower() in {'unknown', 'null', 'nan', 'none', '(null)', '(nan)', '(none)', '未知', '', ' '}:
        content_2 = ''
...
def getParser():
    ...
    parser.add_argument('--localPath', type=str, default='/data1/tmp/', required=False, help='The localPath of input file')
...
class SinglePassClusterSearcher(object):
    def __init__(self, args):
        ...
        self.nullContentList = ['unknown', 'null', 'nan', 'none', '(null)', '(nan)', '(none)', '未知', '', ' ']
        self.stopWords = ['~', '：', '/', '「', '、', '★', ',', '|', '【', '^', '」', '—', '“', '>', '.', '￥', '%', ')', '_', '……', ';', '（', '?', '&', '‘', ' ', '，', '-', '～', '）',
                          '！', '＃', '”', '!', '。', '#', '？', '{', 't', '；', '\\', '】', '+', '*', '《', '$', '…', '=', ']', '[', "'", '·', '(', '"', '’', ':', '》', '→', '<']
    ...
    def clusterResult(self, inparams):
        clusterResult = collections.defaultdict(dict)
        self.keyWordsCntTemp = 0

        inparams_df = pd.DataFrame.from_dict(inparams, orient='index').T

        # dataPreprocess
        inparams_df['geohash_code'] = inparams_df.apply(lambda row: getGeohashCode(row, 'device_lat', 'device_lon'), axis=1)
        inparams_df.drop('device_lat', axis=1, inplace=True)
        inparams_df.drop('device_lon', axis=1, inplace=True)

        inparams_df['create_time_hour'] = inparams_df.apply(lambda row: getSubString(row, 'create_time', 0, 13), axis=1)
        inparams_df.drop('create_time', axis=1, inplace=True)

        inparams_df['cover_phone_number'] = inparams_df.apply(lambda row: getSubString(row, 'user_mobile', 0, 9), axis=1)
        inparams_df.drop('user_mobile', axis=1, inplace=True)

        inparams_df['device_brand_type'] = inparams_df.apply(lambda row: getConcatFeature(row, 'device_brand', 'device_type'), axis=1)
        inparams_df.drop('device_brand', axis=1, inplace=True)
        inparams_df.drop('device_type', axis=1, inplace=True)

        inparams_df['device_brand_type_appversion'] = inparams_df.apply(lambda row: getConcatFeature(row, 'device_brand_type', 'appversion'), axis=1)
        inparams_df.drop('device_brand_type', axis=1, inplace=True)

        inparams_df = inparams_df[self.colOrder]
        inparams = inparams_df.iloc[0, :]

        clusterResult['inparams'] = OrderedDict(inparams)
        clusterResult[self.objectIdName] = str(inparams[self.objectIdName])

        candiUidsWithWeight = {}
        for simhashColName, colContent in inparams[self.simhashColsIndexDict.keys()].items():
            if str(colContent).lower() not in self.nullContentList and len(str(colContent)) >= self.colsMinContentLenDict[simhashColName]:
                inparamsCut = self.getCutContent(str(colContent), self.colsCutStepDict[simhashColName])

                inputStrCutSimhash = Simhash(inparamsCut, f=self.args.simhashDim)
                ...
                self.colsSimhashIndex.add(clusterResult[self.objectIdName], Simhash(inparamsCut, f=self.args.simhashDim), simhashColName, k=int(self.simhashColsTolDict[simhashColName]))

        candiUidsWithWeightSorted = sorted(candiUidsWithWeight.items(), key=lambda item: item[1], reverse=True)
        clusterResult['clusterid'], clusterResult['clustercnt'], simValue, objectFlag = self.search(clusterResult[self.objectIdName], str(inparams[self.ClusterIdTail]), candiUidsWithWeightSorted)
        ...
        if objectFlag == 'repeatObject':
            ...
            self.UpdateCoreWord(inparams, clusterResult['clusterid'])
    ...
    def executeModel(self, inContent):
        inparams = json.loads(inContent, object_pairs_hook=OrderedDict)
        try:
            if str(inparams[self.objectIdName]).lower() in self.nullContentList:
                logging.info('The id is null\n')

                clusterResultDict = collections.defaultdict(dict)
                clusterResultDict['inparams'] = OrderedDict(inparams)
                clusterResultDict[self.objectIdName] = str(inparams[self.objectIdName])
                clusterResultDict['reason'] = "The objectId is null."
                self.recordsCnt += 1
                clusterResultDict['clusterscore'] = 0.0

            else:
                clusterResultDict = self.clusterResult(inparams)
...
pt = sys.argv[1]
two_weeks_datetime = datetime.datetime.strptime(pt, '%Y%m%d') - datetime.timedelta(days=13)
...
sqlStr = '''
    ...
    (
        select distinct signer_up_table.seqno
        ...
        (
            ...
            select distinct * from 
            (
                select distinct get_json_object(data,'$.uid') uid
...
try:
    ...
    if len(searchData.take(1)) > 0:
        try:
            searchData = searchData.toPandas()
            logging.info("Getting SinglePassClusterSearcher instance")
            singlePassSearch = SinglePassClusterSearcher(args)

            clusterScoreResult = []
            for index, row in searchData.iterrows():
                inContent = searchData.iloc[index, :].to_json()
                clusterResultDict = singlePassSearch.executeModel(inContent)

                if index % singlePassSearch.args.showFrequency == 0:
                    logging.info("index: %d" % index)

                if clusterResultDict['clusterscore'] > singlePassSearch.args.scoreThreshold and singlePassSearch.keyWordsCntTemp != len(singlePassSearch.keyWordsList):
                    clusterResultJson = json.dumps(clusterResultDict, ensure_ascii=False)
                    clusterScoreResult.append([clusterResultDict[singlePassSearch.objectIdName], clusterResultDict['clusterid'], clusterResultDict['clusterscore'], clusterResultJson])

            logging.info('saving clustersResult\n')
            clustersResultTrainDF = pd.DataFrame(clusterScoreResult, columns=[singlePassSearch.objectIdName, 'clusterid', 'clusterscore', 'inputoutputparams'])
            ...
            uid2clusterIdDF = pd.DataFrame([(k, v) for k, v in singlePassSearch.uid2clusterIdDict.items()], columns=[str(singlePassSearch.objectIdName), 'clusterid'])
            ...
            clustersRecordDF = pd.DataFrame([(k, v['memberscnt'], v['simmean'], singlePassSearch.getCorewordStr(v)) for k, v in singlePassSearch.clustersRecordDict.items()], columns=['clusterid', 'memberscnt', 'simmean', 'coreword'])
            ...
            colsSimhashIndexDF = pd.DataFrame([(k, json.dumps(v, ensure_ascii=False)) for k, v in singlePassSearch.colsSimhashIndex.bucket.items()], columns=['simhashkey', 'valueset'])
